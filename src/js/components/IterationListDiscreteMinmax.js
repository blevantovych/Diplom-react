...
export default class IterationListDiscreteMinmax extends React.Component {
    ...
    changeErrPlot = () => {
        ...
        Plotly.animate('plotall_errors_discrete', {
            data: [{
    ...
    render() {
        ...
        const yMin = Math.min(...this.props.arr.map(i => Math.min(...i.error_plot[1])))
        const yMax = Math.max(...this.props.arr.map(i => Math.max(...i.error_plot[1])))
