...
class ComparisonDiscrete extends Component {
    render() {
        ...
        if (this.props.minmaxData) {
            ...
            mmPlot = <Plot id="comp_discrete_mm_plot" legend={false}
                                plotData={[
                                    {x: minmaxData.x_approx, y: minmaxData.approximation, name: 'Апроксимація'},
                                    {x: minmaxData.x_vals, y: minmaxData.y_vals, mode: 'markers', name: 'Табл. дані'},
                                    {x: minmaxData.alternance, y: minmaxData.f_alternance, mode: 'markers', name: 'Точки альтернансу'},
                                    {...minmaxData.max_error_line, name: 'Максимальна похибка'}
                                ]}
                            />
            lsPlot = <Plot id="comp_discrete_ls_plot" legend={false}
                                plotData={[
                                    {
                                    ...
                                    {x: this.props.lssqData.x_vals, y: this.props.lssqData.y_vals, mode: 'markers', name: 'Табл. дані'},
                                    {
        ...
        return (
            ...
            <div>
                ...
                {this.props.lssqData && <div style={{width: '90vw', position: "absolute", left: '-15vw', margin: '30px 0'}}>
                    <Card>
                        <CardText>
                            <Table>
                                <TableBody displayRowCheckbox={false}>
                                    ...
                                    <TableRow>
                                        ...
                                        <TableRowColumn>{minmaxData.max_error.toFixed(4)}</TableRowColumn>
                                        <TableRowColumn>{this.props.lssqData.max_error.toFixed(4)}</TableRowColumn>
                                    </TableRow>
                                    <TableRow>
                                        <TableRowColumn width={'20%'}>x в якому макс похибка</TableRowColumn>
                                        <TableRowColumn>{minmaxData.x_error.toFixed(4)}</TableRowColumn>
                                        <TableRowColumn>{this.props.lssqData.x_error.toFixed(4)}</TableRowColumn>
                                    </TableRow>
                                    <TableRow>
                                        <TableRowColumn width={'20%'}>Аналітичний вигляд</TableRowColumn>
                                        <TableRowColumn><Formula formula={minmaxData.formula.replace(truncateCoefs(4), '$1')}/></TableRowColumn>
                                        <TableRowColumn><Formula formula={this.props.lssqData.formula.replace(truncateCoefs(4), '$1')}/></TableRowColumn>
